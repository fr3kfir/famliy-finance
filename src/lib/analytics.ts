...
export function getTodayStats(transactions: Transaction[]) {
  ...
  const earned   = todayTx.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0);
  return { spent, earned, count: todayTx.length };
}

export function getProjection(transactions: Transaction[], year: number, month: number) {
  const now = new Date();
  const dayOfMonth = now.getDate();
  const daysInMonth = new Date(year, month + 1, 0).getDate();
  const { expenses } = getMonthlyStats(transactions, year, month);
  if (!expenses || !dayOfMonth) return null;
  const projected = Math.round((expenses / dayOfMonth) * daysInMonth);
  return { projected, daysLeft: daysInMonth - dayOfMonth };
}

export function getMonthComparison(transactions: Transaction[]) {
  const now = new Date();
  const curr = getMonthlyStats(transactions, now.getFullYear(), now.getMonth());
  const prev = new Date(now.getFullYear(), now.getMonth() - 1, 1);
  const last = getMonthlyStats(transactions, prev.getFullYear(), prev.getMonth());
  return { curr, last };
}
...
export function getSavingsTips(transactions: Transaction[]): string[] {
  ...
  if (tips.length === 0)
    tips.push('הוסיפו עסקאות לחודש הנוכחי כדי לקבל המלצות מותאמות אישית.');
  return tips;
}
